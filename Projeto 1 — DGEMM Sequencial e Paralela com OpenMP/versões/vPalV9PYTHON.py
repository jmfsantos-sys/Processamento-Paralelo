...
- Gerar tabela comparativa automática (CSV)
...
# -------------------------------
# Geração de tabela comparativa CSV
# -------------------------------

def gerar_tabela_comparativa(results, filename="dgmm_comparativa.csv"):
    colunas = ["Tamanho", "Threads", "Tempo(s)", "GFLOPS", "Speedup", "Eficiência(%)"]
...
def plot_comparativo(results, filename="dgmm_comparativo.pdf"):
    ...
    sns.lineplot(data=df, x="Tamanho", y="Tempo(s)", hue="Threads", style="Threads", markers=True, dashes=False)
    plt.title("DGEMM Python: Comparativo de todas as versões")
    plt.xlabel("Tamanho da matriz")
    plt.ylabel("Tempo (s)")
    plt.legend(title="Versão / Threads")
...
if __name__=="__main__":
    ...
    gerar_tabela_comparativa(results)
